...
function createView() {
  ...
  // forward notes from one window to another
  ipcMain.on('message', (ev, name, notes) => {
    viewWindow.webContents.send('message', name, notes)
  })

  viewWindow.webContents.setWindowOpenHandler((details) => {
    shell.openExternal(details.url)
    return { action: 'deny' }
  })
  ...
  // Load the remote URL for development or the local html file for production.
  if (is.dev && process.env['ELECTRON_RENDERER_URL']) {
    viewWindow.loadURL(`${process.env['ELECTRON_RENDERER_URL']}/view.html`)
  } else {
    viewWindow.loadFile(join(__dirname, '../view/view.html'))
  }
}
