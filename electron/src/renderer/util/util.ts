...
export const changeArrayResolution = (array: any[] | TypedArray, resolution: number) => {
  const resChange = array.length / resolution
  let newArray: number[] = []
  for (let i = 0; i < array.length; i += resChange) {
    const index = Math.floor(i)
    const remainder = i - index
    newArray.push(lerp(array[index], array[index + 1] ?? array[index], remainder))
  }
  return newArray
}
...
export const useEffectState = <T>(func: () => T, cleanup: (input: T) => void, deps: any[] = []) => {
...
export const useRefAsState = <T>(initialValue: T | null): [T, (set: T) => void, () => T] => {
...
export const useStateAsRef = <T>(initialValue: T): [T, (T: T) => void, () => T] => {
...
export function separateGroups(bufGeom) {
