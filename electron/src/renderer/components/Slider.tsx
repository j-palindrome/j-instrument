...
export default function Slider({
  ...
  return (
    <div
      className={`${className} relative h-full flex`}
      ...
      onMouseMove={(ev) => {
        if (clicking && rect.current) {
          ...
          const y = (ev.clientY - rect.current.y) / rect.current.height
    ...
    >
      <div className={`${innerClassName} absolute bottom-0 left-0`} ref={slider}></div>
      <div className="h-full w-4" onClick={() => setTimeout(() => onEnd({ x: 0, y: 0 }))}></div>
      <div className="h-full w-full flex justify-center items-center select-none mix-blend-difference">
        {children}
      </div>
      <div className="h-full w-4" onClick={() => setTimeout(() => onEnd({ x: 1, y: 1 }))}></div>
    </div>
  )
}
