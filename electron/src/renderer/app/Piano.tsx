...
export default function Piano() {
  ...
  useEventListener(
    ...
    (ev: KeyboardEvent) => {
      ...
      velocity.current = scale(key, 0, velocityKeys.length, 1 / velocityKeys.length, 1)
    },
    []
  )
  ...
  useEventListener(
    ...
    (ev: KeyboardEvent) => {
      ...
      const currentNoteIndex = notes.findIndex((synth) => synth.velocity && synth.key === key)
      ...
      newNotes[voiceIndex] = { started, key, velocity: velocity.current, value: DUMMY }
  ...
  const onChange = useCallback(([low, high]) => setters.setScale({ range: [low, high] }), [])

  const sliderSetters = useRef({ setMin: (n: number) => {}, setMax: (n: number) => {} })
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex w-full space-x-2">
        ...
        <Link to="industrialization">industrialization</Link>
