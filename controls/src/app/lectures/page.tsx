...
export default function Lectures() {
  const newNote = useAppStore(
    useShallow(state => sortBy(state.notes, x => -x.started)[0])
  )
  const newVoice = useAppStore(
    useShallow(state => state.notes.indexOf(newNote))
  )
  ...
  useEffect(() => {
    ...
    let startTime =
      scale(newNote.value, MIDI_BOTTOM, MIDI_TOP, 0, 1) * totalTime
    ...
    if (endsAfter === -1) {
      ...
      window.max?.outlet('/selectBuffer', `setvalue ${newVoice} set speeches.1`)
    } else {
  ...
  useEffect(() => {
    window.max?.bindInlet('/totalTime', (index: number, length: number) => {
      totalTime.current[index] = length
      timeSums.current = totalTime.current.map((x, i) =>
        _.sum(totalTime.current.slice(0, i + 1))
      )
    })
  }, [])
    ...
    <div>
      <select
        onChange={ev => {
          ...
          window.max?.outlet('/selectedBuffer', val)
        }}>
