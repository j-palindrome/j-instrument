...
export default function Industrialization() {
  const newNote = useAppStore(
    useShallow(state => maxBy(state.notes, x => x.started)!)
  )
  const newVoice = useAppStore(
    useShallow(state => state.notes.indexOf(newNote) + 1)
  )
  ...
  useEffect(() => {
    ...
    let startTime =
      ((newNote.value + MIDI_BOTTOM) / (MIDI_TOP - MIDI_BOTTOM)) * totalTime

    // find the recording which ends before it
    const endsAfter = _.findLastIndex(
      timeSums.current,
      ending => ending < startTime
    )

    if (endsAfter === -1) {
      // it's in the first recording
      window.max?.outlet('/selectBuffer', `setvalue ${newVoice} set sounds.1`)
    } else {
    ...
    window.max?.outlet(
      ...
      `setvalue ${newVoice} ${
        !newNote.velocity
          ? `stop`
          : `start ${startTime} ${scale(newNote.velocity, 0.2, 1, 0.3, 3, 2)}`
      }`
    )
  }, [newNote])
  ...
  useEffect(() => {
    ...
    //   '/totalTime',
    //   (_ev, index: number, length: number) => {
    ...
    <div>
      ...
      <div className='flex space-x-2'>
        <input
          ...
          step={0.01}
          onChange={ev => {
            window.max?.outlet('/speed', ev.target.value)
          }}></input>
        <button onClick={() => window.max?.outlet('/speed', 1)}>1</button>
      </div>
      ...
      <div className='flex space-x-2'>
        <input
          ...
          step={0.01}
          onChange={ev => {
            window.max?.outlet('/ted', ev.target.value)
          }}></input>
        <button onClick={() => window.max?.outlet('/ted', 1)}>1</button>
        <button onClick={() => window.max?.outlet('/ted', 0)}>0</button>
      </div>
    </div>
  )
}
