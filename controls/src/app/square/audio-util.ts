...
const xor = (notes: number[], i: number): [number, number] => {
  ...
  const x = xPoints[0]
    ? math.round(math.mod(i, xPoints[0]) / (xPoints[1] ?? xPoints[0]))
    : 0
  const y = yPoints[0]
    ? math.round(math.mod(i, yPoints[0]) / (yPoints[1] ?? yPoints[0]))
    : 0
  const z = zPoints[0]
    ? math.round(math.mod(i, zPoints[0]) / (zPoints[1] ?? zPoints[0]))
    : 0
