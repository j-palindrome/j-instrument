...
import DashBrush from '@libs/asemic/src/DashBrush'
import { hash, range, time, vec4 } from 'three/src/nodes/TSL'
...
function App() {
  ...
  const textures = [
    ...
    img11,
    img12
  ].map(tex => new TextureLoader().load(tex))

  return (
    <>
      <AsemicCanvas outputChannel={ctx => getChannel(ctx, socket)}>
        <Asemic>
          {({ h }) => (
            <>
              ...
              <LineBrush
                onInit={g =>
                  g
                    .newText('a\nmans\nface')
                    .setCurves('all', { left: 0, middle: 0.75 * h, width: 0.5 })
                }></LineBrush>
              <LineBrush
                renderInit
                maxLength={5}
              ...
              <LineBrush
                ...
                onInit={g => {
                  g.newText(
                    ...
                    { top: 0.85 * h, center: 0.5, width: 0.7 },
                    i =>
                      g.transform({
